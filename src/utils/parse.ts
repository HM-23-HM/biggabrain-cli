...
export function fixKatexSyntax(jsonStrings: string[]): string[] {
  ...
  const singleSlashCommands =
    /\[tex\]([^[]*?)\\(frac|sqrt|vec|text|begin|end)\b/g;
  ...
  const matrixBreaks =
    /\[tex\]([^[]*?)(?<!\\)(\\)(?!frac|sqrt|vec|text|begin|end)(?! )/g;
  ...
  return jsonStrings.map((original) => {
    ...
    while ((match = texRegexGlobal.exec(fixed)) !== null) {
      ...
      const replacedOutside = outsideChunk.replace(
        dollarSignOutsideMath,
        "\\$"
      );
    ...
    const remainingOutside = fixed
      .slice(lastIndex)
      .replace(dollarSignOutsideMath, "\\$");
...
export function extractQuestionAndSolution(
...
export function wrapTexContentWith(
  input: string,
...
export function extractAndSaveJsonStrings(
  input: string,
...
export function chopUpDis(fileContent: string, delimiter: string = "***"){
  const first = replaceDemtingyah(fileContent, delimiter)
  return replaceJsonCommas(first, `\n${delimiter}\n`)
}

export function replaceDemtingyah(fileContent: string, delimiter: string = ","): string {
  return fileContent
    .split("\n")
    .map((line) => {
      ...
      // Replace standalone ```json
      if (trimmedLine === "```json") return delimiter;
      // Replace standalone ``````json
      if (trimmedLine === "``````json") return delimiter;
      // Remove lines with ``` that don't match ```json or ``````json
      if (
        trimmedLine.startsWith("```") &&
        !trimmedLine.match(/^(```json|``````json)$/)
      )
        return "";
      // Replace lines containing ```json but not exactly ```json or ``````json
      if (
...
export function replaceJsonCommas(text: string, replacement = '\n###\n') {
  return text.replace(/\},\s*[\r\n\s]*\{/g, `}${replacement}{`);
}

export function processJsonTextAgain(fileContent: string): string {
  const temp = replaceDemtingyah(fileContent);
  return addBrackets(temp);
}
